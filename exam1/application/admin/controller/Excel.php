...
class Excel extends Controller
{
    ...
    public function expData($table_name=''){
        //获取表注释
        $table_notes = D($table_name)->query(
            "select TABLE_COMMENT from information_schema.TABLES where TABLE_NAME='".$table_name."'
            and TABLE_SCHEMA='".C('DB_NAME')."'");

        $xlsName  = "{$table_notes[0]['table_comment']}数据表";
        //获取每个字段的注释
        $sql =" show full fields from ".$table_name;
        $fileds = D($table_name)->query($sql); //$fields中存放着每一个字段的信息

        $xlsCells = array();
        foreach ($fileds as $filed){
            $xlsCells[$filed['field']] = str2arr($filed['comment'],'@')[0];
        }
        $xlsCell = array();
        $i = 0;
    ...
    public function impUser($table_name='')
    {
        if (!empty($_FILES['file']['name'])) {
            ...
            if (strtolower($exts) == "xlsx" || strtolower($exts) == "xls") {
                ...
                if ($info) {
                    vendor("PHPExcel.PHPExcel");
                    $file_name = './Uploads/excel/'.$info['file']['savepath'] . $info['file']['savename'];
                    //判断文件是什么格式
    ...
    private function merge($other,$table_name){
        ...
        $arr_continue = array('id','major_id','right_key','major_id','subject_id','add_time','status');
        $sql =" show full fields from ".$table_name;
        $fields = D($table_name)->query($sql); //$fields中存放着每一个字段的信息
        ...
        foreach ($other as $k=>$v){
            ...
            $insert_datas[] = array_combine($arr,$v);
            //跳过导入表格的第一行和第二行
    ...
    private function inert_data($insert_datas,$table_name=''){
        ...
        $result = D($table_name)->addAll($insert_datas);
        if($result){
            return true;
        }else{
            return false;

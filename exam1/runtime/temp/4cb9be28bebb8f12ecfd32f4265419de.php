    ...
    <script>
      ...
      function member_del(obj,id){
          layer.confirm('确认要彻底删除吗？',function(index){
              //ajax请求 id数组
              $.post("<?php echo url('Students/remove'); ?>",{id:id},function (data) {
                  if(data.status == 1){
                      //发异步删除数据
      ...
      function member_trash(obj,id){
          layer.confirm('确认恢复吗？',function(index){
              $.post("<?php echo url('Students/recover'); ?>",{id:id},function (data) {
                  if(data.status == 1){
                      //发异步恢复数据
                      $(obj).parents("tr").remove();
                      layer.msg(data.msg,{icon:1,time:1000});
      ...
      function delAll (argument) {
        ...
        layer.confirm('确认要恢复吗？'+ids.toString(),function(index){
            //ajax请求 id数组
            $.post("<?php echo url('Students/recover'); ?>",{id:ids},function (data) {
                if(data.status == 1){
                    //捉到所有被选中的，发异步进行删除
